...
export async function createDetector(updateStatus, updateMessage) {
  ...
  if (runtime === 'mediapipe') {
    return faceDetection.createDetector('MediaPipeFaceDetector', {
      runtime,
      modelType: 'short',
      maxFaces: 1,
      solutionPath:
 ...
 */
 export async function detectFacesV2(model, camera) {

  try {
    let faces =
        await model.estimateFaces(camera.video, {flipHorizontal: false});
    return faces;
  } catch (error) {
    model.dispose();
    model = null;
